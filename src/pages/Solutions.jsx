 ...
 *
 * Motion is deliberately restrained: one thing moves at a time. The five
 * business types travel sideways while the section is pinned, the impact
 * figures count up once, and hovers stay short. Phones get the simpler
 * version — a stacked list, no pinning.
 */
...
function SolutionsHero() {
  return (
    <section className="relative flex flex-col justify-center overflow-hidden bg-white pb-16 pt-28 md:pb-20 lg:h-[100svh] lg:min-h-[42rem] lg:pt-24">
      <div
        aria-hidden
        ...
        style={{
          ...
          maskImage: "radial-gradient(ellipse 75% 65% at 40% 45%, #000 25%, transparent 78%)",
          WebkitMaskImage: "radial-gradient(ellipse 75% 65% at 40% 45%, #000 25%, transparent 78%)",
        }}
      />
      ...
      <div className="relative mx-auto w-full max-w-[84rem] px-5 md:px-10">
        ...
        <h1 className="mt-4 max-w-4xl text-[clamp(2.1rem,min(4.6vw,8vh),4.4rem)] font-extrabold leading-[1.03] tracking-[-0.045em] text-jet">
          <span className="block overflow-hidden pb-1">
            <span className="wipe block">End-to-end supply chain,</span>
          </span>
          <span className="block overflow-hidden pb-1">
            <span className="wipe block" style={{ animationDelay: ".1s" }}>
              built for <span className="bg-gradient-to-r from-brand-blue to-brand-green bg-clip-text text-transparent">healthcare.</span>
            </span>
          </span>
        </h1>

        <p className="rise mt-6 max-w-2xl text-[clamp(1rem,2vh,1.18rem)] leading-relaxed text-ink-soft" style={{ animationDelay: ".25s" }}>
          {SOLUTIONS_HERO.sub}
        </p>

        <div className="rise mt-8 flex flex-wrap gap-2.5" style={{ animationDelay: ".35s" }}>
          {SOLUTIONS_HERO.pills.map((pill) => (
            <span
              key={pill}
              className="rounded-full border border-hairline bg-white px-4 py-2 text-[0.88rem] font-semibold text-ink-soft"
            >
              {pill}
            </span>
          ))}
        ...
        <div className="rise mt-10 flex flex-wrap items-center gap-3" style={{ animationDelay: ".45s" }}>
          <Magnetic>
            <Link
              to="/partner"
          ...
          <a
            href="#who"
            className="rounded-full border border-hairline bg-white px-7 py-3.5 text-[0.98rem] font-bold text-jet transition-colors duration-300 hover:border-brand-blue"
          >
...
function SolutionPanel({ item, index, total }) {
  return (
    <article className="flex h-full w-full shrink-0 items-center">
      <div className="mx-auto grid w-full max-w-[84rem] items-center gap-8 px-5 md:px-10 lg:grid-cols-[1fr_1.05fr] lg:gap-14">
        ...
        <figure className="relative aspect-[4/3] overflow-hidden rounded-[2rem] bg-jet lg:aspect-auto lg:h-[min(56svh,30rem)]">
          <img src={ART[item.art]} alt="" className="h-full w-full object-cover" />
...
function AudiencesPinned() {
  ...
  const goTo = (i) => {
    ...
    window.scrollTo({ top: top + (travel * i) / (total - 1), behavior: "smooth" });
  };
        ...
        <div className="mt-6 flex min-h-0 flex-1 items-center">
          <motion.div className="flex h-full w-full" style={{ x }}>
            {SOLUTION_TABS.map((item, i) => (
              <SolutionPanel key={item.id} item={item} index={i} total={total} />
            ))}
          </motion.div>
        </div>

        {/* Tabs double as the progress track. */}
...
/** Phones and tablets: the same content, stacked. No pinning, no sideways travel. */
  ...
  return (
    <section className="bg-white py-20 md:py-28">
      <div className="mx-auto max-w-[84rem] px-5 md:px-10">
        ...
        <div className="mt-12 grid grid-cols-2 border-l border-t border-hairline lg:grid-cols-4">
          {SOLUTIONS_IMPACT.stats.map((stat, i) => (
            <Reveal key={stat.label} from="up" delay={(i % 4) * 0.06}>
              <div className="group h-full border-b border-r border-hairline p-6 transition-colors duration-300 hover:bg-floral md:p-8">
                <p className="text-[clamp(2rem,4vw,3.2rem)] font-extrabold leading-none tracking-[-0.05em] text-jet transition-colors duration-300 group-hover:text-brand-blue">
...
export default function Solutions() {
  ...
  return (
    <>
      <SolutionsHero />
      {desktop && !reduce ? <AudiencesPinned /> : <AudiencesStacked />}
      <Impact />

      {/* Closing call to action */}
      <section className="bg-jet py-20 text-white md:py-24">
        <div className="mx-auto flex max-w-[84rem] flex-col items-start justify-between gap-8 px-5 md:flex-row md:items-center md:px-10">
          <h2 className="max-w-2xl text-[clamp(1.8rem,3.4vw,2.8rem)] font-extrabold leading-[1.08] tracking-[-0.04em]">
            One network behind every order you promise.
          </h2>
          <Magnetic>
            <Link
              to="/partner"
              className="group flex shrink-0 items-center gap-3 rounded-full bg-brand-green py-2 pl-7 pr-2 text-[1rem] font-extrabold text-jet transition-colors duration-300 hover:bg-white"
            >
              Partner with us
